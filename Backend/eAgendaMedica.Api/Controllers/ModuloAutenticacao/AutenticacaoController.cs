...
namespace eAgendaMedica.Api.Controllers.ModuloAutenticacao
{
    ...
    public class AutenticacaoController : ControllerBase
    {
        ...
        private static string CriarChaveToken(Usuario usuario, DateTime dataExpiracao) //Monta a chave
        {
            ...
            var segredo = Encoding.ASCII.GetBytes("SegredoeAgendaMedica");//chave da criptografia

            var algoritmo = SecurityAlgorithms.HmacSha256Signature;

            var chaveSimetrica = new SymmetricSecurityKey(segredo);// chave compartilhada entre o emissor (quem assina) e o receptor (quem verifica a assinatura)
